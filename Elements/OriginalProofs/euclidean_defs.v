...
Definition 中点 A B C := BetS A B C /\ Cong A B B C.
Definition 等角 A B C a b c := exists U V u v, Out B A U /\ Out B C V /\ Out b a u /\ Out b c v /\ Cong B U b u /\ Cong B V b v /\ Cong U V u v /\ nCol A B C.
...
Definition 垂直于 P Q A B C := exists X, Col P Q C /\ Col A B C /\ Col A B X /\ Per X C P.
Definition Perp P Q A B := exists X, 垂直于 P Q A B X.
...
Definition RT A B C D E F := exists X Y Z U V, Supp X Y U V Z /\ 等角 A B C X Y U /\ 等角 D E F V Y Z.
...
Definition SumA A B C D E F P Q R := exists X, 等角 A B C P Q X /\ 等角 D E F X Q R /\ BetS P X R.
