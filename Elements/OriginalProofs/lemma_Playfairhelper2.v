...
{
 let Tf:=fresh in
 assert (Tf:exists m, (BetS A m C /\ BetS E m B)) by (conclude_def CR );destruct Tf as [m];分离合取式.
 ...
 let Tf:=fresh in
 assert (Tf:exists e, (BetS E C e /\ Cong C e E C)) by (conclude lemma_extension);destruct Tf as [e];分离合取式.
 ...
 assert (Tf:exists H, (BetS B H m /\ BetS A H M)) by (conclude postulate_Pasch_inner);destruct Tf as [H];分离合取式.
 ...
 let Tf:=fresh in
 assert (Tf:exists F, (BetS A F e /\ BetS E m F)) by (conclude postulate_Pasch_outer);destruct Tf as [F];分离合取式.
 ...
 let Tf:=fresh in
 assert (Tf:exists K, (BetS B K C /\ BetS e K F)) by (conclude postulate_Pasch_inner);destruct Tf as [K];分离合取式.
 assert (BetS e F A) by (conclude axiom_betweennesssymmetry).
