...
intros.
let Tf:=fresh in
assert (Tf:exists K, (BetS E F K /\ Cong F K G H /\ TG A B C D E K)) by (conclude_def TT );destruct Tf as [K];分离合取式.
let Tf:=fresh in
assert (Tf:exists J, (BetS A B J /\ Cong B J C D /\ Lt E K A J)) by (conclude_def TG );destruct Tf as [J];分离合取式.
let Tf:=fresh in
assert (Tf:exists L, (BetS P Q L /\ Cong Q L R S /\ TG E F G H P L)) by (conclude_def TT );destruct Tf as [L];分离合取式.
let Tf:=fresh in
assert (Tf:exists M, (BetS E F M /\ Cong F M G H /\ Lt P L E M)) by (conclude_def TG );destruct Tf as [M];分离合取式.
