...
intros.
let Tf:=fresh in
assert (Tf:exists E F G, (Col A B E /\ Col A B F /\ BetS Q E G /\ BetS R F G /\ nCol A B Q /\ nCol A B R)) by (conclude_def OS );destruct Tf as [E[F[G]]];分离合取式.
assert (TS Q A B G) by (conclude_def TS ).
assert (TS P A B G) by (conclude lemma_planeseparation).
let Tf:=fresh in
assert (Tf:exists M, (BetS P M G /\ Col A B M /\ nCol A B P)) by (conclude_def TS );destruct Tf as [M];分离合取式.
