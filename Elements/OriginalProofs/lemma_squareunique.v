...
assert (等角 E A B D A B) by (conclude lemma_Euclid4).
assert ((Cong E B D B /\ 等角 A E B A D B /\ 等角 A B E A B D)) by (conclude proposition_04).
assert (等角 A B D A B E) by (conclude lemma_equalanglessymmetric).
...
assert (等角 A B M A B E) by (conclude lemma_equalanglestransitive).
assert (nCol A B E) by (forward_using lemma_NCorder).
assert (等角 A B E A B E) by (conclude lemma_equalanglesreflexive).
assert (等角 A B E A B N) by (conclude lemma_equalangleshelper).
assert (等角 A B M A B N) by (conclude lemma_equalanglestransitive).
...
assert (等角 B C E B C D) by (conclude lemma_Euclid4).
...
assert ((Cong B E B D /\ 等角 C B E C B D /\ 等角 C E B C D B)) by (conclude proposition_04).
assert (neq B C) by (forward_using lemma_NCdistinct).
assert (eq C C) by (conclude cn_equalityreflexive).
assert (Out B C C) by (conclude lemma_ray4).
assert (nCol B C D) by (conclude lemma_rightangleNC).
assert (nCol C B D) by (forward_using lemma_NCorder).
assert (等角 C B D C B D) by (conclude lemma_equalanglesreflexive).
assert (等角 C B D C B M) by (conclude lemma_equalangleshelper).
assert (nCol C B E) by (forward_using lemma_NCorder).
assert (等角 C B E C B E) by (conclude lemma_equalanglesreflexive).
assert (等角 C B E C B N) by (conclude lemma_equalangleshelper).
assert (等角 C B E C B D) by (conclude lemma_equalanglestransitive).
assert (等角 C B D C B E) by (conclude lemma_equalanglessymmetric).
assert (等角 C B M C B D) by (conclude lemma_equalanglessymmetric).
assert (等角 C B M C B E) by (conclude lemma_equalanglestransitive).
assert (等角 C B M C B N) by (conclude lemma_equalanglestransitive).
assert (等角 M B C N B C) by (conclude lemma_equalanglesflip).
