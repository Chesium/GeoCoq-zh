...
assert (~ Meet A B D C).
 ...
 intro.
 let Tf:=fresh in
 assert (Tf:exists T, (neq A B /\ neq D C /\ Col A B T /\ Col D C T)) by (conclude_def Meet );destruct Tf as [T];分离合取式.
...
assert (~ Meet B A C D).
 ...
 intro.
 let Tf:=fresh in
 assert (Tf:exists T, (neq B A /\ neq C D /\ Col B A T /\ Col C D T)) by (conclude_def Meet );destruct Tf as [T];分离合取式.
...
assert (~ Meet B A D C).
 ...
 intro.
 let Tf:=fresh in
 assert (Tf:exists T, (neq B A /\ neq D C /\ Col B A T /\ Col D C T)) by (conclude_def Meet );destruct Tf as [T];分离合取式.
 assert (Col A B T) by (forward_using lemma_collinearorder).
 assert (Col C D T) by (forward_using lemma_collinearorder).
 assert (Meet A B C D) by (conclude_def Meet ).
 contradict.
...
let Tf:=fresh in
assert (Tf:exists Q P R, (Col A B P /\ Col A B R /\ BetS C P Q /\ BetS D R Q /\ nCol A B C /\ nCol A B D)) by (conclude_def OS );destruct Tf as [P[Q[R]]];分离合取式.
close.
Qed.
