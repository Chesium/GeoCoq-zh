...
assert (角度小于 D C B B D A) by (conclude proposition_16).
assert (~ eq B C).
 {
 intro.
...
assert (等角 A C B A C B) by (conclude lemma_equalanglesreflexive).
assert (等角 A C B D C B) by (conclude lemma_equalangleshelper).
assert (角度小于 A C B B D A) by (conclude lemma_angleorderrespectscongruence2).
assert (等角 A D B B D A) by (conclude lemma_ABCequalsCBA).
assert (角度小于 A C B A D B) by (conclude lemma_angleorderrespectscongruence).
assert (等角 A B D A D B) by (conclude lemma_equalanglessymmetric).
assert (角度小于 A C B A B D) by (conclude lemma_angleorderrespectscongruence).
assert (~ Col B C A).
 {
 intro.
 assert (Col A B C) by (forward_using lemma_collinearorder).
...
assert (角度小于 B C A A B D) by (conclude lemma_angleorderrespectscongruence2).
...
assert (角度小于 A B D A B C) by (conclude_def 角度小于 ).
assert (角度小于 B C A A B C) by (conclude lemma_angleordertransitive).
close.
Qed.
