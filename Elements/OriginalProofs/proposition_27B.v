...
let Tf:=fresh in
assert (Tf:exists B, (BetS A E B /\ Cong E B A E)) by (conclude lemma_extension);destruct Tf as [B];分离合取式.
assert (neq F D) by (forward_using lemma_angledistinct).
assert (neq D F) by (conclude lemma_inequalitysymmetric).
let Tf:=fresh in
assert (Tf:exists C, (BetS D F C /\ Cong F C F D)) by (conclude lemma_extension);destruct Tf as [C];分离合取式.
