...
let Tf:=fresh in
assert (Tf:exists F, (BetS B A F /\ Cong A F A B)) by (conclude lemma_extension);destruct Tf as [F];分离合取式.
...
let Tf:=fresh in
assert (Tf:exists C, (Per B A C /\ TS C B F R)) by (conclude proposition_11B);destruct Tf as [C];分离合取式.
...
let Tf:=fresh in
assert (Tf:exists D, (Out A C D /\ Cong A D A B)) by (conclude lemma_layoff);destruct Tf as [D];分离合取式.
assert (Out A D C) by (conclude lemma_ray5).
assert (eq A A) by (conclude cn_equalityreflexive).
assert (Col A B A) by (conclude_def Col ).
let Tf:=fresh in
assert (Tf:exists q, (BetS C q R /\ Col B F q /\ nCol B F C)) by (conclude_def TS );destruct Tf as [q];分离合取式.
...
let Tf:=fresh in
assert (Tf:exists G e M, (BetS G D e /\ 等角 G D A D A B /\ Par G e F B /\ BetS G M B /\ BetS D M A)) by (conclude proposition_31short);destruct Tf as [G[e[M]]];分离合取式.
...
let Tf:=fresh in
assert (Tf:exists E, (PG D E B A /\ Col G e E)) by (conclude lemma_triangletoparallelogram);destruct Tf as [E];分离合取式.
...
let Tf:=fresh in
assert (Tf:exists p, (BetS G D p /\ Cong G D p D /\ Cong G A p A /\ neq D A)) by (conclude_def Per );destruct Tf as [p];分离合取式.
...
let Tf:=fresh in
assert (Tf:exists d, (BetS E d G /\ Col D A d /\ nCol D A E)) by (conclude_def TS );destruct Tf as [d];分离合取式.
