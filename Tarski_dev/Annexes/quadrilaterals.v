...
Lemma midpoint_中点蕴含共线 : forall A B A' B' M,
 A <> B ->
 中点 M A A' -> 中点 M B B' ->
 Col A B B' ->
...
assert(HH:=midpoint_中点蕴含共线 B' A' B A M).
...
intro.
subst M.
apply A是AB中点则A与B重合 in H9.
contradiction.

assert(A <> M).
intro.
subst M.
apply A是AB中点则A与B重合 in H8.
contradiction.
...
right.
分离合取式.
assert(HH:=两中点组全段等长则前半段等长 A M B C M D H0 H4 H1).
assert(A = C \/ 中点 M A C).
...
intros.
assert (HH := H).
unfold 严格平行四边形 in HH; 分离合取式.
destruct H1; auto; 分离合取式.
...
assert(HH:= 中点蕴含等长_uniqueness A D C B M H7 H6 H8).
induction HH.
分离合取式.
contradiction.
分离合取式.
...
apply (中点蕴含等长_uniqueness A D B C M).
Col.
assumption.
assumption.
induction H5.
